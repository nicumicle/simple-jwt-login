...
$auth0Domain = $jwtSettings->getIntegrationsSettings()->auth0()->getDomain();
echo esc_url($auth0Domain ? sprintf('https://%s/authorize', $auth0Domain) : '#');
?>"
      class="simple-jwt-login-oauth-app auth0">
    <input type="hidden" name="client_id" value="<?php echo esc_attr($jwtSettings->getIntegrationsSettings()->auth0()->getClientId());?>" />
