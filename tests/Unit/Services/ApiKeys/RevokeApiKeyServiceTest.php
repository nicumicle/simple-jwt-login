...
class RevokeApiKeyServiceTest extends TestCase
{
    /**
    ...
    public function testThrowsWhenNotLoggedIn()
    {
        ...
        $service = (new RevokeApiKeyService())
            ->withRequest(['id' => 1])
    ...
    public function testNonAdminCannotRevokeOtherUsersKey()
    {
        $this->expectException(Exception::class);
        $this->expectExceptionCode(ErrorCodes::ERR_API_KEY_UNAUTHORIZED);
    ...
    public function testNonAdminCanRevokeOwnKey()
    {
        $mock = $this->createStub(WordPressDataInterface::class);
        $mock->method('getOptionFromDatabase')->willReturn(json_encode([]));
        $mock->method('isUserLoggedIn')->willReturn(true);
        ...
        $repoMock->method('revokeById')->willReturn(true);
    ...
    public function testThrowsWhenRevokeFails()
    {
        $this->expectException(Exception::class);
        $this->expectExceptionCode(ErrorCodes::ERR_API_KEY_REVOKE_FAILED);

        $this->apiKeyRepositoryMock->method('revokeById')->willReturn(false);
    ...
    {
        $this->apiKeyRepositoryMock->method('revokeById')->willReturn(true);
