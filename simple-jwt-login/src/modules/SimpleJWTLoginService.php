...
class SimpleJWTLoginService
{
        ...
        if (! empty($this->jwt_settings->getAllowedLoginIps())) {
            ...
            if (! in_array(
                ...
                array_map(
                    ...
                    explode(
                        ',',
                        $this->jwt_settings->getAllowedLoginIps()
                    )
                )
            )
            ) {
    ...
    public function validateRegisterUser()
    {
        ...
        if (!isset($this->request['email'])
            || ! isset($this->request['password']) && $this->jwt_settings->getRandomPasswordForCreateUser() === false
        ) {
            throw new Exception(
        ...
        if (! empty($this->jwt_settings->getAllowedRegisterDomain())) {
            ...
            if (! isset($parts[1]) || isset($parts[1]) && ! in_array($parts[1], array_map(
                'trim',
    ...
    public function createUser()
    {
        ...
        if (isset($this->request[$this->jwt_settings->getAuthCodeKey()])) {
            ...
            foreach ($authCodes as $code) {
                $authCodeBuilder = new AuthCodeBuilder($code);
                if ($authCodeBuilder->getCode() === $this->request[$this->jwt_settings->getAuthCodeKey()]
    ...
    public function deleteUser()
    {
        ...
        if ($this->jwt_settings->getRequireDeleteAuthKey()
            && ! isset($this->request[ $this->jwt_settings->getAuthCodeKey() ])
        ) {
            throw new Exception(
    ...
    public function authenticateUser()
    {
        ...
        foreach ($this->jwt_settings->getJwtPayloadParameters() as $parameter) {
            if ($parameter === SimpleJWTLoginSettings::JWT_PAYLOAD_PARAM_IAT
    ...
    public function refreshJwt()
    {
        ...
        if (isset($payload[ SimpleJWTLoginSettings::JWT_PAYLOAD_PARAM_EXP ])) {
            ...
            $payload[ SimpleJWTLoginSettings::JWT_PAYLOAD_PARAM_EXP ] = time()
                + ($this->jwt_settings->getAuthJwtTtl() * 60);
        }
