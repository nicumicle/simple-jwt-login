...
{
    private function make(): FacebookOauthSettings
    {
        return new FacebookOauthSettings();
    }
    ...
    {
        $this->assertSame('facebook', $this->make()->getGroup());
    }

    public function testGetName(): void
    {
        $this->assertSame('Facebook', $this->make()->getName());
    }
    ...
    {
        $post   = ['facebook' => ['enable_exchange_token' => '1']];
    ...
    {
        $result = $this->make()->processPost(['facebook' => []], $this->wpData());
    ...
    {
        return [
            'no features enabled'   => [['enabled' => 1], 12011],
            'client_id missing'     => [['enabled' => 1, 'enable_oauth' => 1], 12012],
            'client_secret missing' => [['enabled' => 1, 'enable_oauth' => 1, 'client_id' => 'fb-id'], 12013],
            'redirect_uri missing'  => [
                ['enabled' => 1, 'enable_exchange_code' => 1, 'client_id' => 'fb-id', 'client_secret' => 'fb-secret'],
                12014,
            ],
        ];
    }
    ...
    public function testValidateThrows(array $groupPost, int $expectedCode): void
    {
        ...
        $this->make()->validate(['facebook' => $groupPost]);
    }
}
